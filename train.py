...
def main(config):
    ...
    model = config.init_obj(config["arch"], module_arch, config["arch"]["args"])
    logger.info(model)
    ...
    if config["lr_scheduler"]["type"] == "OneCycleLR":
        scheduler_kwargs["steps_per_epoch"] = config["trainer"].get("len_epoch", len(dataloaders["train"]))
        scheduler_kwargs["epochs"] = config["trainer"]["epochs"]

    lr_scheduler = config.init_obj(config["lr_scheduler"], torch.optim.lr_scheduler, optimizer, **scheduler_kwargs)
